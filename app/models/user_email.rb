...
class UserEmail < ActiveRecord::Base
  ...
  def self.email_with_hash(user)
    id_hash = Digest::MD5.hexdigest(user.id)[0, 6]
    address = "autogenerated@#{user.first_name}-#{id_hash}-#{user.last_name}.com"
    address
  end
  ...
  def replace_autogenerated_email_if_no_email
    if is_primary
      if UserEmail.where(user_id: user, is_primary: true).size == 1 && UserEmail.find_by(user_id: user, is_primary: true).id != id && user.no_email? == true

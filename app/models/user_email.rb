...
class UserEmail < ActiveRecord::Base
  ...
  before_validation :replace_autogenerated_email_if_no_email, on: [:create]
  ...
  def autogenerated?
    ...
    autogenerated = true if "autogenerated@#{user.first_name}-#{user.id}-#{user.last_name}.com".downcase.match(address.downcase).present?
    autogenerated
  end
  ...
  def replace_autogenerated_email_if_no_email
    if is_primary
      if UserEmail.where(user_id: user, is_primary: true).size == 1 && UserEmail.find_by(user_id: user, is_primary: true).id != id && UserEmail.find_by(user_id: user, is_primary: true).autogenerated? && user.no_email? == true
        user.class.set_no_email(user.id, false, user)
        change_to_primary_email
      end
    end
  end

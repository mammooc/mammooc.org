...
class User < ApplicationRecord
  ...
  def self.find_for_omniauth(auth, signed_in_resource = nil)
    ...
    if user.nil?
      ...
      if user.nil?
        ...
        user = User.new(
          full_name: full_name.presence || autogenerated,
          profile_image: nil,
        ...
        begin
          user.profile_image = process_uri(auth.info.image || auth.info.picture_url)
    ...
    elsif email.present? && !user.emails.pluck(:address).include?(email.downcase)
    ...
    if identity.user != user && user.present?
      identity.user = user
      identity.save!
    end
    user
  ...
  def full_name_autogenerated?
    autogenerated = false
    UserIdentity.where(user: self).find_each do |identity|
      autogenerated = true if "autogenerated@#{identity.provider_user_id}-#{identity.omniauth_provider}.com".downcase.match(full_name.downcase).present?
    end
    autogenerated
  end

  def primary_email_autogenerated?
    emails.find_by(address: primary_email).autogenerated?
  end

...
module Users
  class OmniauthCallbacksController < Devise::OmniauthCallbacksController
    ...
    def self.provides_callback_for(provider)
      ...
      def #{provider}
        ...
        if @user.present? && env["omniauth.params"].blank?
          ...
          sign_in_and_redirect @user, event: :authentication
          set_flash_message(:notice, :success, kind: "#{provider}".to_s) if is_navigational_format? && !current_user_logged_in
        else
    ...
    [:Facebook, :Google, :github, :LinkedIn, :Twitter, :WindowsLive, :Amazon, :openHPI].each do |provider|
      ...
      if current_user.identities.count > 1 || !current_user.password_autogenerated
        begin
          ...
          flash['error'] << if deauthorize_params[:provider] == 'easyID'
                              ...
                              t('users.settings.identity_not_deleted', provider: deauthorize_params[:provider].to_s)
                            end
        else
          flash['success'] << if deauthorize_params[:provider] == 'easyID'
                                t('users.settings.easyID.identity_deleted')
                              else
                                t('users.settings.identity_deleted', provider: deauthorize_params[:provider].to_s)
                              end
        end
      else
        flash['error'] << if deauthorize_params[:provider] == 'easyID'

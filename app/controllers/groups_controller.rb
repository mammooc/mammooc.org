...
class GroupsController < ApplicationController
  ...
  def initial_passwords
    @sorted_group_users = sort_by_name(@group.users - admins)
    @initial_passwords = @group.initial_passwords
    @group_picture = Group.group_images_hash_for_groups [@group]
  end
  ...
  def create
    ...
    respond_to do |format|
      if @group.save
        ...
        if params[:group][:student_list].present?
          register_batch
        end
      else
  ...
  def register_batch
    student_list = params[:group][:student_list]
    students = student_list.split(/[,;]/)
    initial_group_passwords = {}
    students.each do |student|
      names = student.split(" ")
      last_name = names.last
      first_name = names[0...-1].join(" ")
      user_password = Devise.friendly_token[0, 8]
      user = User.new(
      ...
      address = UserEmail.email_with_hash(user)
      email = UserEmail.new(
        user: user,
        is_primary: true,
        is_verified: false,

...
RSpec.describe UserDatesController, type: :controller do
  ...
  let(:valid_attributes) { {user_id: user.id, course_id: course.id, mooc_provider_id: mooc_provider.id, date: Time.zone.now, title: 'Assignment 1', kind: 'submission deadline', relevant: true} }
  ...
  describe 'GET events_for_calendar_view' do
    it 'assigns dates from user in the specified time period to @current_user_dates' do
      user_date = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now + 1.day)
    ...
    it 'does not assign dates from user which are not in the specified time period to @current_user_dates' do
      user_date = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now + 1.day)
      old_user_date = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now - 1.day)
      future_user_date = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now + 5.days)
      get :events_for_calendar_view, start: Time.zone.now, end: Time.zone.now + 2.days, format: :json
      expect(assigns(:current_user_dates)).to eq([user_date])
  ...
  describe 'GET synchronize_dates_on_dashboard' do
    ...
    it 'assings the first three current dates of user to @current_dates_to_show' do
      user_date1 = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now)
      user_date2 = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now + 1.day)
      user_date3 = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now + 4.days)
      user_date4 = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now + 8.days)
      expect(UserDate).to receive(:synchronize).with(user).and_return(true).once
      get :synchronize_dates_on_dashboard, format: :json
    ...
    it 'does not assign an old date to @current_dates_to_show' do
      old_user_date = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now - 1.day)
      FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now)
      expect(UserDate).to receive(:synchronize).with(user).and_return(true).once
      get :synchronize_dates_on_dashboard, format: :json
  ...
  describe 'GET create_calendar_feed' do
    it 'renders calendar feed including user_date' do
      user_date = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now)
  ...
  describe 'GET my_dates' do
    ...
    it 'renders calendar feed for correct user' do
      ...
      user_date = FactoryGirl.create(:user_date, user: user, course: course, mooc_provider: mooc_provider, date: Time.zone.now, title: 'correct event')
      user_date2 = FactoryGirl.create(:user_date, user: user2, course: course, mooc_provider: mooc_provider, date: Time.zone.now, title: 'wrong event')
      get :my_dates, format: :ics, token: user.token_for_user_dates
      expect(response.body).to include(user_date.title)

...
RSpec.describe Users::OmniauthCallbacksController, type: :controller do
  ...
  describe 'deauthorize' do
    ...
    it 'is not allowed to remove the existing OmniAuth Connection if it is the only one' do
      ...
      expect(flash['error']).to include(I18n.t('users.settings.identity_not_deleted', provider: identity.omniauth_provider.to_s))
    end
    ...
    it 'is allowed to remove the OmniAuth connection if the user chose a password' do
      ...
      user.save!
      get :deauthorize, provider: identity.omniauth_provider
      expect(flash['success']).to include(I18n.t('users.settings.identity_deleted', provider: identity.omniauth_provider.to_s))
    end
    ...
    it 'returns an error flash message if the connection was not destroyed' do
      ...
      expect(flash['error']).to include(I18n.t('users.settings.identity_not_deleted', provider: 'not exisiting'.to_s))
    end
  end
end

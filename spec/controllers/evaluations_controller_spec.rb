    ...
    context 'export_course_evaluations' do
      context 'specifies course' do
        ...
        it 'returns all evaluations for specific course and specific provider' do
          get :export_course_evaluations, format: :json, provider: mooc_provider.name, course_id: course.provider_course_id
        ...
        it 'returns first page of evaluations for specific course and specific provider' do
          get :export_course_evaluations, format: :json, provider: mooc_provider.name, course_id: course.provider_course_id, per_page: 2, page: 1
          expect(subject.first['rating']).to eql evaluation.rating
          expect(subject.second['rating']).to eql evaluation2.rating
        ...
        it 'returns specific page of evaluations for specific course and specific provider' do
          get :export_course_evaluations, format: :json, provider: mooc_provider.name, course_id: course.provider_course_id, per_page: 1, page: 3
      ...
      context 'error' do
        ...
        it 'raises error if only course_id is given' do
          get :export_course_evaluations, format: :json, course_id: course.provider_course_id
          expect(subject).to include 'no provider given for the course'
        end

        it 'returns an error if specific course is not present ' do
          get :export_course_evaluations, format: :json, provider: mooc_provider.name, course_id: '12345678901'
          expect(subject).to eql "Couldn't find Course"
        end

        it 'returns an error if specific provider is not present ' do
          get :export_course_evaluations, format: :json, provider: 'assdaddsdad', course_id: course.provider_course_id
    ...
    context 'export_overall_course_rating' do
      context 'specifies course' do
        ...
        it 'returns the number of all evaluations for specific course and specific provider' do
          get :export_overall_course_rating, format: :json, provider: mooc_provider.name, course_id: course.provider_course_id
          expect(subject['number_of_evaluations']).to eql 3
        end

        it 'returns overall rating for specific course and specific provider' do
          get :export_overall_course_rating, format: :json, provider: mooc_provider.name, course_id: course.provider_course_id, per_page: 1
      ...
      context 'error' do
        ...
        it 'raises error if only course_id is given' do
          get :export_overall_course_rating, format: :json, course_id: course.provider_course_id
          expect(subject).to include 'no provider given for the course'
        end

        it 'returns an error if specific course is not present ' do
          get :export_overall_course_rating, format: :json, provider: mooc_provider.name, course_id: '12345678901'
          expect(subject).to eql "Couldn't find Course"
        end

        it 'returns an error if specific provider is not present ' do
          get :export_overall_course_rating, format: :json, provider: 'assdaddsdad', course_id: course.provider_course_id
          expect(subject).to eql "Couldn't find MoocProvider"
        end
      end
    end

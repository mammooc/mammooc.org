...
RSpec.describe GroupsController, type: :controller do
  ...
  describe 'invite members' do
    ...
    it 'does nothing if there are no members to invite' do
      post :invite_group_members, as: :json, params: { id: group_with_admin.id, members: '' }
    ...
    it 'splits invite members string correctly to email array' do
      ...
      post :invite_group_members, as: :json, params: { id: group_with_admin.id, members: email_string }
    ...
    it 'invites members' do
      expect { post :invite_group_members, as: :json, params: { id: group_with_admin.id, members: members } }.to change { GroupInvitation.count }.by(2)
      expect(response.body).to have_content('"error_email":[]')
    ...
    it 'returns wrong email addresses' do
      ...
      expect { post :invite_group_members, as: :json, params: { id: group_with_admin.id, members: email_string } }.to change { GroupInvitation.count }.by(3)
  ...
  describe 'POST condition for changing member status' do
    ...
    it "returns 'last_admin' if the member is the last admin (but there are still other members)" do
      UserGroup.set_is_admin(group.id, user.id, true)
      post :condition_for_changing_member_status, as: :json, params: { id: group.id, changing_member: user.id }
    ...
    it "returns 'last_member' if the member is the last member" do
      ...
      post :condition_for_changing_member_status, as: :json, params: { id: second_group.id, changing_member: user.id }
      expect(json).to have_content('last_member')
    end

    it "returns 'ok' if there are no restrictions to remove the member" do
      UserGroup.set_is_admin(group.id, user.id, true)
      post :condition_for_changing_member_status, as: :json, params: { id: group.id, changing_member: second_user.id }
  ...
  describe 'GET members' do
    ...
    it 'returns JSON with all members exclude the current user' do
      get :members, as: :json, params: { id: group.id }
  ...
  describe 'GET groups_where_user_is_admin' do
    ...
    it 'returns all groups where current_user is admin' do
      get :groups_where_user_is_admin, as: :json

...
RSpec.describe UserEmail, type: :model do
  ...
  describe 'change_to_primary_email' do
    ...
    it 'changes the is_primary attribute to true and changes the existing primary_email.is_primary to false' do
      email2.change_to_primary_email
      expect(UserEmail.find(email2.id).is_primary).to be true
      expect(UserEmail.find(email1.id).is_primary).to be false
      expect(described_class.where(user: user, is_primary: true).count).to eql 1
    end

    it 'changes is_primary of an already primary_email' do
      email1.change_to_primary_email
      expect(UserEmail.find(email2.id).is_primary).to be false
      expect(UserEmail.find(email1.id).is_primary).to be true
      expect(described_class.where(user: user, is_primary: true).count).to eql 1
    end
  end
end

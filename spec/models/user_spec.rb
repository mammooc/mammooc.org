...
RSpec.describe User, type: :model do
  ...
  describe 'self.find_for_omniauth' do
    ...
    it 'associates the mooc provider for the user' do
      FactoryBot.create(:mooc_provider, oauth_strategy_name: 'openhpi')
      user = FactoryBot.create(:OmniAuthUser)
      UserIdentity.find_by(user: user)
      authentication_info = OmniAuth::AuthHash.new(
        provider: 'openhpi',
        ...
        credentials: {
          token: 'token',
          expires: true,
          expires_at: 0, # 01/01/1970
          refresh_token: 'refresh'
        }
      )
      identity = UserIdentity.where(user: user).count
      mooc_provider_connection = MoocProviderUser.where(user: user).count
      expect { described_class.find_for_omniauth(authentication_info, user) }.not_to(change { described_class.count })
      expect(identity + 1).to eq UserIdentity.where(user: user).count
      expect(mooc_provider_connection + 1).to eq MoocProviderUser.where(user: user).count
    end
  end

...
RSpec.describe UserIdentity, type: :model do
  describe 'find_for_omniauth' do
    ...
    it 'finds the user identiy if present' do
      ...
      expect(described_class.find_for_omniauth(authentication_info)).to eql identity
    end
    ...
    it 'returns a new user identiy if no existing could be found' do
      ...
      expect(new_identity.omniauth_provider).to eql authentication_info.provider
      expect(new_identity.provider_user_id).to eql authentication_info.uid

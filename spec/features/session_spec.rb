...
RSpec.describe 'Users::Session', type: :feature do
  ...
  it 'shows finish sign up page if no full name is provided (e.g. when using OmniAuth) and saves the input' do
    user = FactoryBot.create(:OmniAuthUser)
    ...
    click_button 'submit_sign_in'
    expect(page).to have_text(I18n.t('users.sign_in_up.finish_sign_up'))
    fill_in 'full_name_finish_sign_up', with: 'Maxi Musterfrau'
    click_button 'submit_finish_sign_up'
    expect(user.reload.full_name).to eq 'Maxi Musterfrau'
    expect(page).to have_text(I18n.t('flash.notice.users.successfully_updated'))
  end

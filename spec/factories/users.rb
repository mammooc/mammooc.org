...
FactoryGirl.define do
  ...
  factory :OmniAuthUser, class: User do
    ...
    after(:create) do |user|
      ...
      create(:user_email, user: user, address: "autogenerated@#{identity.provider_user_id}-#{identity.omniauth_provider}.com")
    end
    after(:stub) do |user|
      identity = create(:user_identity, user: user)
      user.emails = [build_stubbed(:user_email, user: user, address: "autogenerated@#{identity.provider_user_id}-#{identity.omniauth_provider}.com")]
      allow(user).to receive(:primary_email).and_return(user.emails.first.address)
    end
  end
end
